...
Exquisitus exports one component, a feature grid. `FeatureGrid` is the theme's own splash layout, which acts as an editorial alternative to `CardGrid`'s uniform grid. Import it in any MDX page:

```mdx
import FeatureGrid from 'starlight-theme-exquisitus/components/FeatureGrid.astro';
```

`FeatureGrid` wraps Starlight's `<Card>` components and accepts a `layout` prop (`'lead'` or `'alternating'`). See the [Feature Grid](/showcase/feature-grid/) showcase for live examples and arrangement options.

## Design system

This theme was developed using the [Impeccable](https://impeccable.style/) design system. You can find its files, `DESIGN.md` and `PRODUCT.md` in the repository root. Food for thought, or inspiration?
