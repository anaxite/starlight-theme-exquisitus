...
Renders the editorial end mark: a small centred terminal that closes each content page's prose, in the tradition of a printed article's end sign.
It never appears on the splash, whose colophon already closes that page.
Set it to `false` to omit the mark entirely.
